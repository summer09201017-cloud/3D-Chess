// js/puzzles.js(全域 script,非 module;垂直搬運自 3d-chess-co)- 📅 每日殘局題庫(N 步殺)
//
...
// 每天出幾題(一組)。★ 5 題=一次坐下來解得完、又有「今天全解」的成就感。
...
/* ★ 每日一組(0831 使用者點名「不要只有 1 題」):
     從題庫決定性抽 count 題**不重複**,再依 mateIn 由易到難排 ⇒
     今天全世界拿到的是同一組、同一個順序;一天一組=一次能坐下來解好幾題。
   ★ count 超過題庫大小時自動夾住(不會抽出重複題目)。 */
function puzzlesForDate(key, count = DAILY_SET_SIZE) {
  ...
  // 決定性 Fisher-Yates,取前 n 個(比「抽到重複就重抽」穩:次數固定、無迴圈風險)
  for (let i = pool.length - 1; i > 0; i -= 1) {
    const j = Math.floor(rng() * (i + 1));
    [pool[i], pool[j]] = [pool[j], pool[i]];
  }
  const indexes = pool.slice(0, n)
    .sort((a, b) => DAILY_PUZZLES[a].mateIn - DAILY_PUZZLES[b].mateIn || a - b);   // 由易到難;同難度用索引穩定排序
  return { key, indexes, puzzles: indexes.map((i) => DAILY_PUZZLES[i]) };
}

// ★ 這站的 js/ 是傳統 script(非 module)⇒ 全域命名空間交件。
window.ChessDaily = { DAILY_PUZZLES, DAILY_SET_SIZE, dailyPuzzleKey, puzzleForDate, puzzlesForDate };
